 ...
 */
export async function runMigrations(): Promise<{ appliedCount: number; results: MigrationRecord[] }> {
  console.log('🔄 Iniciando verificación de migraciones PostgreSQL...');
  await ensureMigrationsTable();
  const migrationsDir = resolveMigrationsDir();
  const files = fs.readdirSync(migrationsDir).filter(f => f.endsWith('.sql')).sort();

  const appliedRes = await pool.query('SELECT version FROM schema_migrations WHERE success = true');
  const appliedVersions = new Set(appliedRes.rows.map(r => r.version));

  let appliedCount = 0;
  const executionResults: MigrationRecord[] = [];
