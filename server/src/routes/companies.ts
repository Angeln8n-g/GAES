...
companiesRouter.post("/", async (req: Request, res: Response) => {
  try {
    ...
    await pool.query(
      ...
      [companyId, cleanName, cleanSlug, cleanLogo, cleanRnc, cleanIndustry, cleanEmail, cleanPhone, active]
    );
    ...
    await pool.query(
      `UPDATE companies 
       SET name = $1, 
           ...
           is_active = $8
       WHERE id = $9`,
      [cleanName, cleanSlug, cleanLogo, cleanRnc, cleanIndustry, cleanEmail, cleanPhone, active, id]
    );
