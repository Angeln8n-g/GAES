 ...
 */
export async function fetchFullEvents() {
  ...
  for (const evt of eventsResult.rows) {
    ...
    const schedulesResult = await pool.query(
      `SELECT * FROM event_schedules WHERE event_id = $1 ORDER BY date ASC`,
      [evt.id]
    );
    ...
    for (const sch of schedulesResult.rows) {
      ...
      const slotsResult = await pool.query(
        `SELECT * FROM event_slots WHERE schedule_id = $1 ORDER BY id ASC`,
        [sch.id]
      );
    ...
    const notifsResult = await pool.query(
      `SELECT to_char(date, 'YYYY-MM-DD HH12:MI AM') as date, channel, status, recipients 
       FROM notification_logs WHERE event_id = $1 ORDER BY date DESC`,
      [evt.id]
    );
...
eventsRouter.post('/', async (req: Request, res: Response) => {
  ...
  try {
    ...
    await client.query(
      `INSERT INTO events (id, title, description, category, instructor, image_url, status, send_email, send_teams, custom_message, modality, location, survey_url)
    ...
    for (const sch of event.schedule || []) {
      const schRes = await client.query(
        `INSERT INTO event_schedules (event_id, date)
         VALUES ($1, $2)
      ...
      for (const sl of sch.slots || []) {
        await client.query(
          `INSERT INTO event_slots (schedule_id, time, capacity)
           VALUES ($1, $2, $3)
           ON CONFLICT (schedule_id, time) DO UPDATE SET capacity = EXCLUDED.capacity`,
          [scheduleId, sl.time, sl.capacity]
        );
      }
    }
