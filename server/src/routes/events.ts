 ...
 */
export async function fetchFullEvents(companyId?: string) {
  ...
  for (const evt of eventsResult.rows) {
    ...
    const notifsResult = await pool.query(
      `SELECT channel, status, recipients, date 
  ...
  try {
    ...
    for (const sch of event.schedule || []) {
      ...
      for (const sl of sch.slots || []) {
        ...
        await client.query(
          `INSERT INTO event_slots (schedule_id, time, end_time, capacity, checkin_code, checkout_code)
           VALUES ($1, $2, $3, $4, $5, $6)
           ON CONFLICT (schedule_id, time) DO UPDATE SET 
             ...
             checkout_code = COALESCE(event_slots.checkout_code, EXCLUDED.checkout_code)`,
          [scheduleId, sl.time, sl.endTime || null, sl.capacity, checkinCode, checkoutCode]
        );
      }
    }
