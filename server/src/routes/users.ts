    ...
    for (const u of users) {
      ...
      if (existing.rows.length > 0) {
        await client.query(
          ...
          [cleanName, role, password, cedula, department, assignedMemberCards, employmentStatus, isActive, companyId, cleanEmail]
        );
      } else {
        await client.query(
          `INSERT INTO users_simulated (id, email, name, role, password, cedula, department, assigned_member_cards, employment_status, is_active, company_id)
          ...
          [userId, cleanEmail, cleanName, role, password, cedula, department, assignedMemberCards, employmentStatus, isActive, companyId]
        );
      }
      ...
      await client.query(
        `UPDATE participants
         SET name = $1,
             ...
             company_id = $6
         WHERE LOWER(email) = $7`,
        [cleanName, cedula, department, employmentStatus, isActive, companyId, cleanEmail]
      );
    }
    ...
    await client.query(
      `UPDATE users_simulated 
       SET name = $1, 
           ...
           company_id = $10
       WHERE id = $11`,
      [cleanName, cleanEmail, cleanRole, password ? password.trim() : '', cleanCedula, cleanDept, cleanCards, cleanEmpStatus, cleanIsActive, cleanCompanyId, id]
    );
