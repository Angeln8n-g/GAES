...
router.get('/', async (req: Request, res: Response) => {
  try {
    const result = await pool.query('SELECT key, value, description, updated_at, updated_by FROM system_settings');
...
router.patch('/ojt', async (req: Request, res: Response) => {
  try {
    ...
    const currentResult = await pool.query("SELECT value FROM system_settings WHERE key = 'ojt_plan_90d'");
