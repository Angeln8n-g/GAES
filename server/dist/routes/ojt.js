...
router.post('/checklists', async (req, res) => {
    try {
        ...
        if (!computedStatus) {
            if (score >= 85 && safetyProtocolPass && firstTimeFixPass)
...
router.delete('/checklists/:id', async (req, res) => {
    try {
        const { id } = req.params;
        await db_1.pool.query('DELETE FROM ojt_checklists WHERE id = $1', [id]);
...
router.post('/calibrations', async (req, res) => {
    try {
        const { id, companyId, title, date, conductedBy, participantsReviewed, averageTheoryScore, averageFieldScore, keyFindings, actionAgreements } = req.body;
...
router.get('/metrics', async (req, res) => {
    ...
    `;
        const [ojtRes, calibRes, weakRes] = await Promise.all([
            db_1.pool.query(ojtStatsQuery, compParam),
            db_1.pool.query(calibQuery, compParam),
            db_1.pool.query(weaknessesQuery, compParam)
        ]);
