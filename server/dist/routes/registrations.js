...
exports.registrationsRouter.post('/', async (req, res) => {
    ...
    `;
        ...
        // 5. Insertar registro
        await client.query('INSERT INTO registrations (slot_id, participant_card) VALUES ($1, $2)', [slotId, participantCard]);
...
exports.registrationsRouter.post('/bulk', async (req, res) => {
    ...
    `;
        ...
        for (const rawEmail of emails) {
            ...
            if (partRes.rows.length === 0) {
                ...
                participantCard = `${Math.floor(1000 + Math.random() * 9000)}`;
                await client.query('INSERT INTO participants (card, name, email, cedula) VALUES ($1, $2, $3, $4)', [participantCard, userName, email, userCedula]);
            }
            else {
                participantCard = partRes.rows[0].card;
            }
...
exports.registrationsRouter.post('/assign', async (req, res) => {
    ...
    `;
        ...
        for (const rawEmail of emails) {
            ...
            // Inscribir
            await client.query(`INSERT INTO registrations (slot_id, participant_card, is_mandatory, assigned_by, assignment_type, assignment_notes, assigned_at)
         VALUES ($1, $2, $3, $4, $5, $6, CURRENT_TIMESTAMP)`, [slotId, participantCard, effectiveMandatory, effectiveAssignedBy, effectiveAssignmentType, notes || null]);
            assignedCount++;
        }
...
exports.registrationsRouter.delete('/', async (req, res) => {
    ...
    `;
        ...
        const regCheck = await client.query('SELECT is_mandatory, assigned_by FROM registrations WHERE slot_id = $1 AND participant_card = $2', [slotId, participantCard]);
        ...
        await client.query('DELETE FROM registrations WHERE slot_id = $1 AND participant_card = $2', [slotId, participantCard]);
