      ...
      >
        ...
        <div className="p-6 overflow-y-auto flex-1 space-y-6">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 bg-slate-50 p-3.5 rounded-2xl border border-slate-200/80 text-xs">
            <div>
              <span className="text-slate-600 text-[10px] uppercase font-bold tracking-wider block">Facilitador</span>
              <span className="font-semibold text-slate-800 truncate block">{cohort.facilitatorName || 'Sin asignar'}</span>
            </div>
            <div>
              <span className="text-slate-600 text-[10px] uppercase font-bold tracking-wider block">Grupo Técnico</span>
              <span className="font-semibold text-slate-800 truncate block">{cohort.groupName || 'Abierto'}</span>
            </div>
            <div>
              <span className="text-slate-600 text-[10px] uppercase font-bold tracking-wider block">Horario Diario</span>
              <span className="font-semibold text-slate-800 truncate block">{cohort.dailyTime}</span>
            </div>
            <div>
              <span className="text-slate-600 text-[10px] uppercase font-bold tracking-wider block">Fecha de Sesión</span>
              <div className="flex items-center gap-1.5 mt-0.5">
                ...
                <input
                  ...
                  className="font-semibold text-slate-800 bg-white border border-slate-300 rounded px-1.5 py-0.5 text-xs focus:ring-1 focus:ring-claro"
                />
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 items-center">
            ...
            <div className="space-y-5">
              ...
              <div className="bg-gradient-to-br from-red-50 to-orange-50/50 border border-red-200/80 rounded-2xl p-5 text-center relative overflow-hidden">
                <div className="flex items-center justify-between text-xs text-claro-700 font-bold uppercase tracking-wider mb-2">
                  ...
                  <button
                    ...
                    className="flex items-center gap-1 text-[11px] px-2.5 py-1 min-h-[32px] rounded-lg bg-white/80 border border-red-200 text-claro-700 hover:bg-white transition-colors font-semibold cursor-pointer"
                ...
                <form onSubmit={handleManualCheckIn} className="space-y-3">
                  <div className="flex gap-2">
                    <input
                      ...
                      className="flex-1 px-3.5 py-2 min-h-[44px] text-xs rounded-xl border border-slate-300 focus:outline-none focus:ring-2 focus:ring-claro/20 focus:border-claro text-slate-900 bg-white"
          ...
          <div className="bg-slate-50/80 rounded-2xl border border-slate-200 p-4">
            ...
            {liveCheckIns.length === 0 ? (
              <div className="text-center py-4 text-xs text-slate-600 italic bg-white/60 rounded-xl border border-dashed border-slate-200">
                  ...
                  >
                    <div className="flex items-center gap-2.5 min-w-0">
                      ...
                      <div className="min-w-0">
                        ...
                        <p className="text-[10px] text-slate-600 font-medium">Carnet: {item.card}</p>
                      </div>
                    </div>
                    <div className="text-right shrink-0">
                      <span className="inline-block px-1.5 py-0.5 rounded text-[9px] font-semibold bg-slate-100 text-slate-700">
                        {item.method}
                      </span>
                      <p className="text-[9px] text-slate-600 font-mono mt-0.5">{item.timestamp}</p>
                    </div>
                  </div>
                ))}
              </div>
        ...
        <div className="px-6 py-3.5 bg-slate-50 border-t border-slate-200 flex items-center justify-between shrink-0">
          ...
          <button
            type="button"
            onClick={onClose}
            className="px-4 py-2 min-h-[44px] text-xs font-bold text-slate-700 hover:bg-slate-200 rounded-xl transition-colors cursor-pointer flex items-center justify-center"
          >
            Cerrar
          </button>
        </div>
