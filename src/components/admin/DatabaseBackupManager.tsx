  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="bg-white rounded-2xl p-5 border border-slate-200 shadow-sm space-y-3">
          ...
          <div>
            ...
            <p className="text-xs text-slate-500 mt-1 flex items-center gap-1.5">
              <span className="font-semibold text-slate-700">{stats?.tableCount || 0}</span> tablas en esquema <code className="text-[11px] bg-slate-100 px-1.5 py-0.5 rounded text-slate-600">public</code>
            </p>
          </div>
        ...
        <div className="bg-white rounded-2xl p-5 border border-slate-200 shadow-sm space-y-3">
          <div className="flex items-center justify-between">
            <span className="text-xs font-bold text-slate-500 uppercase tracking-wider">Respaldos Almacenados</span>
          ...
          <div>
            ...
            <p className="text-xs text-slate-500 mt-1 flex items-center gap-1.5">
              <span className="font-semibold text-slate-700">{backups.length}</span> copias .sql.gz en disco
            </p>
          </div>
          <div className="pt-2 border-t border-slate-100 flex items-center justify-between text-[11px] text-slate-500">
            <span>Política GFS Activa</span>
            <span className="text-slate-700 font-semibold">Max 90 Días</span>
          </div>
        </div>
        ...
        <div className="bg-white rounded-2xl p-5 border border-slate-200 shadow-sm space-y-3">
          <div className="flex items-center justify-between">
            <span className="text-xs font-bold text-slate-500 uppercase tracking-wider">Planificador Nocturno</span>
        ...
        <div className="bg-white rounded-2xl p-5 border border-slate-200 shadow-sm space-y-3">
          <div className="flex items-center justify-between">
            <span className="text-xs font-bold text-slate-500 uppercase tracking-wider">Esquema & Migraciones</span>
      ...
      <div className="flex items-center justify-between gap-4 border-b border-slate-200 pb-2">
        <div className="flex items-center gap-2">
          ...
          >
            ...
            <span className={`px-2 py-0.5 rounded-full text-[10px] font-bold ${
              activeTab === 'migrations' ? 'bg-white/20 text-white' : 'bg-slate-100 text-slate-700'
            }`}>
              {migrations.length}
            </span>
          </button>
        ...
        {activeTab === 'migrations' && (
          <button
            ...
            className="flex items-center gap-1.5 px-3.5 py-2 rounded-xl bg-slate-100 hover:bg-slate-200 text-slate-800 text-xs font-bold transition-all cursor-pointer disabled:opacity-50"
          >
        ...
        <div className="bg-white rounded-3xl border border-slate-200 shadow-sm overflow-hidden">
          {backups.length === 0 ? (
            <div className="p-12 text-center space-y-4">
              ...
              <div>
                <h3 className="text-base font-bold text-slate-800">No hay respaldos almacenados</h3>
                      ...
                      <tr key={b.filename} className="hover:bg-slate-50/60 transition-colors">
                        ...
                        <td className="py-3.5 px-4">
                          <div className="flex items-center gap-1.5">
                            <code className="text-[11px] font-mono bg-slate-100 px-2 py-0.5 rounded text-slate-600 max-w-[130px] truncate" title={b.sha256}>
                        ...
                        <td className="py-3.5 px-5 text-right whitespace-nowrap">
                          <div className="inline-flex items-center gap-1.5">
                            ...
                            <a
                              ...
                              download={b.filename}
                              className="p-1.5 rounded-lg bg-slate-100 hover:bg-slate-200 text-slate-700 transition-colors inline-flex items-center gap-1 text-[11px] font-bold"
                            ...
                            <button
                              ...
                              className="p-1.5 rounded-lg bg-amber-50 hover:bg-amber-100 text-amber-700 transition-colors inline-flex items-center gap-1 text-[11px] font-bold cursor-pointer"
                            ...
                            <button
                              ...
                              className="p-1.5 rounded-lg bg-red-50 hover:bg-red-100 text-red-600 transition-colors inline-flex items-center text-[11px] cursor-pointer"
        ...
        <div className="bg-white rounded-3xl border border-slate-200 shadow-sm overflow-hidden">
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left border-collapse">
              ...
              <tbody className="divide-y divide-slate-100 text-xs">
                {migrations.map((m) => (
                  <tr key={m.version} className="hover:bg-slate-50/60 transition-colors">
                    ...
                    <td className="py-3 px-4">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="font-semibold text-slate-900 font-mono text-[11px]">{m.name}</span>
                      </div>
                    </td>
                    <td className="py-3 px-4">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-xs">
          <div className="bg-white rounded-3xl max-w-lg w-full p-6 sm:p-8 shadow-2xl border border-slate-200 space-y-6 animate-in fade-in zoom-in-95 duration-150">
            ...
            <div className="flex items-center gap-3 text-amber-600">
              ...
              <div>
                <h3 className="text-xl font-black text-slate-900">Confirmar Restauración</h3>
                <p className="text-xs text-slate-500 mt-0.5">Operación crítica de base de datos</p>
              </div>
            </div>
            ...
            <div className="bg-amber-50 border border-amber-200 rounded-2xl p-4 text-xs text-amber-800 space-y-2">
              ...
              <ul className="list-disc list-inside space-y-1 text-amber-700 pl-1">
                <li>Se aplicará la copia: <strong className="font-mono text-slate-900">{backupToRestore.filename}</strong>.</li>
            ...
            <div className="space-y-2">
              <label className="block text-xs font-bold text-slate-700">
                Para confirmar, escriba <span className="text-red-600 font-mono">RESTAURAR</span> a continuación:
              </label>
              <input
                type="text"
                ...
                className="w-full px-4 py-2.5 border border-slate-300 rounded-xl text-xs font-bold text-slate-900 focus:outline-none focus:ring-2 focus:ring-amber-500 uppercase font-mono"
              />
            </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-xs">
          <div className="bg-white rounded-3xl max-w-md w-full p-6 shadow-2xl border border-slate-200 space-y-5">
            <div className="flex items-center gap-3 text-red-600">
              ...
              <div>
                <h3 className="text-lg font-black text-slate-900">Eliminar Respaldo</h3>
                <p className="text-xs text-slate-500">Esta acción no se puede deshacer</p>
              </div>
            </div>

            <p className="text-xs text-slate-600">
              ¿Está seguro de que desea eliminar el archivo <strong className="font-mono text-slate-800">{backupToDelete.filename}</strong>? Se borrarán sus metadatos y sumas de verificación.
            </p>
